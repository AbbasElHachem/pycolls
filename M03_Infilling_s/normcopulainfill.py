...
from math import ceil, log as mlog, factorial as fac
...
class NormCopulaInfill:
    ...
    norm_cop_infill_log_%s.log: text
        All activity written on screen is also savedf in this file.
    ...
    def __init__(self,
        ...
        self.in_coords_df = self.in_coords_df.loc[:, ['X', 'Y']]
        ...
        if skip_stns is not None:
            ...
            self.skip_stns = map(unicode, list(skip_stns))
        ...
        if self.verbose:
            ...
            print 'with \'in_coords_df\':', self.in_var_df.shape
            print 'INFO: \'in_coords_df\' shape after station name ',
            print 'intersection with \'in_var_df\':', self.in_coords_df.shape
        ...
        self.out_var_infill_stns_file = os_join(self.out_dir,
                                             'infilled_var_df_infill_stns.csv')
    ...
    def cmpt_plot_nrst_stns(self):
        ...
        for infill_stn in self.infill_stns:
            ...
            for nrst_stn in dists_df.index[1:self.n_max_nebs + 1]:
                # if the infill_stn is already a neighbor of nrst_stn
                if nrst_stn in self.nrst_stns_dict.keys():
                    if infill_stn in self.nrst_stns_dict[nrst_stn]:
                        curr_nebs_list.append(nrst_stn)
                        continue

                _cond_1 = nrst_stn not in curr_nebs_list
                _cond_2 = False
                _cond_3 = False

                if _cond_1:
                    _ = self.in_var_df.loc[:, [infill_stn, nrst_stn]].dropna()
                    _cond_2 = (_.shape[0] >= self.min_valid_vals)
                    _cond_3 = False

                    for infill_date in self.infill_dates:
                        if infill_date in _.index:
                            _cond_3 = True
                            break

                if _cond_1 and _cond_2 and _cond_3:
                    curr_nebs_list.append(nrst_stn)
            ...
            if len(_) >= self.n_min_nebs:
                ...
                self._as_err(('Neighboring stations less than '
                              '\'n_min_nebs\' '
                              'for station: %s') % infill_stn)
                if self.dont_stop_flag:
                    bad_stns_list.append(infill_stn)
                    bad_stns_neighbors_count.append(len(_))
                else:
                    raise RuntimeError
        ...
        for infill_stn in self.infill_stns:
            assert infill_stn in self.in_var_df.columns, \
                self._as_err(('station %s not in input variable dataframe '
                              'anymore!') % infill_stn)
        ...
        assert date_in_dates, \
            self._as_err('None of the infill dates exist in \'in_var_df\' '
...
#            if not self._dist_cmptd:
#                self.nrst_stns_list = self.in_var_df.columns.tolist()
#                for infill_stn in self.infill_stns:
#                    self.nrst_stns_dict[infill_stn] = \
#                        (self.in_var_df.columns.drop(infill_stn)).tolist()
            ...
            loop_stns_df = DataFrame(index=self.infill_stns,
                                     columns=self.in_var_df.columns,
                                     dtype=bool)
            loop_stns_df[:] = False
            ...
            for i_stn in self.infill_stns:
                ...
                assert len(ser_i.shape) == 1, \
                    self._as_err('ser_i has more than one column!')

                if i_stn in drop_infill_stns:
                    continue
                ...
                for j_stn in self.nrst_stns_dict[i_stn]:
                    ...
                    if j_stn in drop_infill_stns:
                        continue

                    try:
                        if loop_stns_df.loc[j_stn, i_stn]:
                            ...
                            if not isnan(self.rank_corrs_df.loc[i_stn, j_stn]):
                                tot_corrs_written += 1
                            loop_stns_df.loc[i_stn, j_stn] = True
                            continue
                    except KeyError:
                        pass
                    ...
                    assert len(ser_j.shape) == 1, \
                        self._as_err('ser_j has more than one column!')
                    ...
                    ### asymmetries
                        ...
                        if (self.ncpus == 1) or self.debug_mode_flag:
                            ...
                            try:
                                asymms_arr = \
                                    array(list(self._norm_cop_pool.map(
                                            self._get_norm_rand_symms,
                                            correls_arr,
                                            nvals_arr)))
                            except:
                        ...
                        assert np_all(isfinite(asymms_arr)), \
                            self._as_err('Invalid values of asymmetries!')
...
#                            'infill stations\' list!') % infill_stn)
        ...
        for infill_stn in self.infill_stns:
            ...
            if not self.force_infill_flag:
                assert curr_n_neighbors >= self.n_min_nebs, \
                    self._as_err(('Rank correlation stations (n=%d) less than '
        ...
        if not self.dont_stop_flag:
            for infill_stn in self.infill_stns:
                assert infill_stn in self.in_var_df.columns, \
                    self._as_err('infill station %s not in input variable '
        ...
        if self.plot_long_term_corrs_flag:
            ...
            for infill_stn in self.infill_stns:
                ...
                plt.savefig(os_join(self.out_long_term_corrs_dir, _),
                            dpi=self.out_fig_dpi, bbox_inches='tight')
                plt.close()
    ...
    def plot_ecops(self):
        ...
        '''
        #TODO: somehow combine with the cmpt_plot_rank_corr_stns ftn
        ...
        if not self._dist_cmptd:
            self.cmpt_plot_nrst_stns()
            assert self._dist_cmptd, \
                self._as_err('Call \'cmpt_plot_nrst_stns\' first!')
...
#            pass
            if not self._rank_corr_cmptd:
                ...
                assert self._rank_corr_cmptd, \
                    self._as_err('Call \'cmpt_plot_rank_corr_stns\' first!')
        else:
            assert False, self._as_err('Incorrect \'nrst_stns_type\': %s' % \
                                       str(self.nrst_stns_type))

        if self.verbose:
            print 'INFO: Plotting empirical copulas of infill stations',
            print 'against others...'
    ...
    def infill(self):
        ...
        assert self._bef_infill_chked, \
            self._as_err('Call \'_before_infill_checks\' first!')
        ...
        if self.verbose:
            print 'INFO: Using %d number of process(es) to infill' % self.ncpus
            print '\n'
            print 'INFO: Flags:'
            self._pretty_print(['debug_mode_flag:', self.debug_mode_flag],
                               nbh=4)
            self._pretty_print(['plot_diag_flag:', self.plot_diag_flag], nbh=4)
            self._pretty_print(['plot_step_cdf_pdf_flag:',
                                self.plot_step_cdf_pdf_flag], nbh=4)
            self._pretty_print(['compare_infill_flag:',
                                self.compare_infill_flag], nbh=4)
            self._pretty_print(['flag_susp_flag:', self.flag_susp_flag], nbh=4)
            self._pretty_print(['force_infill_flag:', self.force_infill_flag],
                               nbh=4)
            self._pretty_print(['plot_neighbors_flag:',
                                self.plot_neighbors_flag], nbh=4)
            self._pretty_print(['take_min_stns_flag:',
                                self.take_min_stns_flag], nbh=4)
            self._pretty_print(['overwrite_flag:', self.overwrite_flag], nbh=4)
            self._pretty_print(['read_pickles_flag:', self.read_pickles_flag],
                               nbh=4)
            self._pretty_print(['use_best_stns_flag:',
                                self.use_best_stns_flag], nbh=4)
            self._pretty_print(['dont_stop_flag:', self.dont_stop_flag], nbh=4)
            self._pretty_print(['plot_long_term_corrs_flag:',
                                self.plot_long_term_corrs_flag], nbh=4)
            self._pretty_print(['save_step_vars_flag:',
                                self.save_step_vars_flag], nbh=4)
            self._pretty_print(['plot_rand_flag:',
                                self.plot_rand_flag], nbh=4)
            print '\n'
        ...
        for ii, infill_stn in enumerate(self.infill_stns):
            ...
            if self.verbose:
                self._pretty_print(nbh=4,
                                   msgs=[('Going through station %d of %d: '
                                          '%s') % (ii + 1,
                                                   self.n_infill_stns,
                                                   infill_stn)])
            ...
            if (not self.overwrite_flag) and os_exists(out_conf_df_file):
                if self.verbose:
                    self._pretty_print([('Output exists already. '
                                         'Not overwriting.')], nbh=8)
            ...
            if self.verbose:
                self._pretty_print(['%d steps to infill' % n_nan_idxs],
                                   nbh=8)
            ...
            if self.nrst_stns_type == 'rank':
                ...
                assert False, \
                    self._as_err('Incorrect \'nrst_stns_type\': %s!' %
                                 self.nrst_stns_type)
            ...
            if self.verbose:
                ...
                self._pretty_print(['Neighbors are:'], nbh=8)
                for i_msg in range(0, len(self.curr_nrst_stns), 3):
                    self._pretty_print(self.curr_nrst_stns[i_msg:(i_msg + 3)],
                                       nbh=12)
            ...
            if ((idxs.shape[0] == 1) or
                ...
                assert n_sub_dates == n_nan_idxs, \
                    self._as_err(('\'n_sub_dates\' (%d) and '
                                  '\'self.infill_dates\' '
                                  '(%d) of unequal length!') %
            ...
            if self.verbose:
                self._pretty_print([('%d steps out of %d infilled' %
                ...
                self._pretty_print([(('Took %0.3f secs, %0.3e secs per '
                                      'step') % (fin_secs,
                                                 fin_secs / n_nan_idxs))],
                                   nbh=8)
            ...
            if plot_infill_cond:
                if self.verbose:
                    self._pretty_print(['Plotting infill...'], nbh=8)
            ...
            if plot_compar_cond and np_any(act_var):
                if self.verbose:
                    self._pretty_print(['Plotting infill comparison...'], nbh=8)
                ...
                if self.verbose:
                    self._pretty_print(['Nothing to compare...'], nbh=8)
            ...
            if plot_flag_susp_cond and np_any(act_var):
                if self.verbose:
                    self._pretty_print(['Plotting infill flags...'], nbh=8)
                ...
                if self.verbose:
                    self._pretty_print(['Nothing to flag...'], nbh=8)
            ...
            if self.verbose:
                self._pretty_print([('Took %0.3f secs for plotting other '
                                     'stuff') % fin_secs], nbh=8)
        ...
        if not self.n_rand_infill_values:
            ...
            _ = self.out_var_df.loc[:, self.infill_stns]
    ...
    def plot_summary(self):
        ...
        for stn_idx in range(cols_per_fig_idxs.shape[0] - 1):
            ...
            infilled_val_clrs = r_2_g_cm(infill_vals / _miss_vals_copy)
            for i, stn in enumerate(colors_df.index):
            ...
            ### compared
            compared_vals = curr_summary_df.loc[:, self._compr_lab].values
            compared_vals[isnan(compared_vals)] = 0.0
            _ = curr_summary_df.loc[:, 'Infilled values'].values.copy()
            _[_ == 0.0] = 1.0
            _[isnan(_)] = 1.0
            compared_vals = compared_vals / _
    ...
    @staticmethod
    def _pretty_print(msgs, nbh=0, nah=0):
...
#                    print 'INFO: \'dont_stop_flag\' set to False!'
    ...
    def _cmpt_conf_ser(self):
        ...
        assert self.conf_probs.shape[0] >= 2, \
            self._as_err('\'_conf_probs\' cannot be less than 2 values!')
        assert self.conf_probs.shape == self.conf_heads.shape, \
            self._as_err(('\'_conf_probs\' (%s) and \'_conf_heads\' (%s) '
        ...
        assert np_any(where(ediff1d(self.conf_probs) > 0, 1, 0)), \
            self._as_err('\'_conf_probs\' not ascending (%s)!' % str(self.conf_probs))

        assert max(self.conf_probs) <= self.adj_prob_bounds[1], \
            self._as_err('max \'adj_prob_bounds\' < max \'_conf_probs\' (%s)!' %
             str(self.adj_prob_bounds))

        assert min(self.conf_probs) > self.adj_prob_bounds[0], \
            self._as_err('min \'adj_prob_bounds\' > min \'_conf_probs\' (%s)!' %
             str(self.adj_prob_bounds))

        assert isfinite(np_all(self.flag_probs)), \
            self._as_err('Invalid values in \'_flag_probs\': %s' %
                         str(self.flag_probs))

        assert isfinite(np_all(self.adj_prob_bounds)), \
            self._as_err(('Invalid values in \'_adj_prob_bounds\': %s') % str(self.adj_prob_bounds))

        assert len(self.adj_prob_bounds) == 2, \
            self._as_err('\'adj_bounds_probs\' are not two values (%s)!' %
             str(self.adj_prob_bounds))

        assert self.adj_prob_bounds[0] < self.adj_prob_bounds[1], \
            self._as_err(('\'adj_bounds_probs\' not ascending (%s)!') %
             str(self.adj_prob_bounds))
        ...
        if self.n_rand_infill_values:
            ...
            assert self.fin_conf_head in self.conf_heads, \
                self._as_err('\'_fin_conf_head\': %s not in \'_conf_heads\': %s' % \
        ...
        if self.flag_susp_flag:
            assert len(self.flag_probs) == 2, \
                self._as_err('Only two values allowed inside \'_flag_probs\'!')
            assert self.flag_probs[0] < self.flag_probs[1], \
                self._as_err('\'_flags_probs\': first value should be smaller '
                             'than the last!')

            for _flag_val in self.flag_probs:
                assert np_any(isclose(_flag_val, self.conf_probs)), \
                    self._as_err('\'_flag_prob\' (%s) value not in \'_conf_probs\' (%s)!' %
    ...
    def __infill(self, infill_dates):
        ...
        def _get_best_stns(best_stns):
            ...
            if infill_date in avail_stns_per_step_dict.keys():
            ...
            for i in range(curr_var_df.shape[1] - 1,
                ...
                n_combs = fac(n_nebs) / (fac(i) * fac(n_nebs - i))
                ...
                for comb in combs:
                    ...
                    if infill_date in avail_stns_per_step_dict.keys():
                        ...
                        if idx is None:
                            _ = avail_stns_per_step_dict.keys()
                            step_idx = _.index(infill_date)
                            for _step in avail_stns_per_step_dict.keys():
            ...
            for col in curr_var_df.columns:
                ...
                if self.infill_type == u'precipitation':
                    ...
                    assert zero_prob <= thresh_prob, \
                        self._as_err('\'zero_prob\' > \'thresh_prob\'!')
                    ...
                    probs_ser = var_ser_copy.rank(method=self._rank_method) / \
                    ...
                    assert thresh_prob <= probs_ser.max(), \
                        self._as_err('\'thresh_prob\' > \'probs_ser.max()\'!')
                else:
                    probs_ser = var_ser.rank(method=self._rank_method) / \
                                             (var_ser.count() + 1.)

                assert np_all(isfinite(probs_ser.values)), \
                    self._as_err('NaNs in \'probs_ser\'!')

                probs_df[col] = probs_ser
                norms_df[col] = norm_ppf_py_arr(probs_ser.values)

                assert np_all(isfinite(norms_df[col].values)), \
                    self._as_err('NaNs in \'norms_df[%s]\'!' % col)
        ...
        def _get_full_corr(full_corrs_arr):
            ...
            if len(del_rows) > 0:
                ...
                if self.verbose:
                    print '\n'
                    self._pretty_print(['WARNING: A correlation of almost '
                                        'equal to one is encountered'],
                                       nbh=8,
                                       nah=8)

                    self._pretty_print(['Infill_stn:', self.curr_infill_stn],
                                       nbh=12)
                    self._pretty_print(['Stations with correlation too high:'],
                                       nbh=12)

                    n_hi_corr = len(too_hi_corr_stns)
                    for i_msg in range(0, n_hi_corr, min(2, n_hi_corr)):
                        self._pretty_print(too_hi_corr_stns[i_msg:(i_msg + 2)],
                                           nbh=16)

                    self._pretty_print(['Infill_date:', infill_date], nbh=12)
                    self._pretty_print(['These stations are added to the '
                                        'temporary high correlations list '
                                        'and not used anymore'],
                                       nbh=12)
                    self._pretty_print([''], nbh=24, nah=24)
                    print '\n'
        ...
        def _fill_u_t_and_get_val_cdf_ftn():
            ...
            for i, col in enumerate(curr_var_df.columns):
                ...
                assert not isnan(_curr_var_val), \
                    self._as_err('_curr_var_val is NaN!')
            ...
            assert np_all(isfinite(u_t)), self._as_err('NaNs in \'u_t\'!')
        ...
        def _get_ppt_stuff():
            ...
            assert val_arr.shape[0], self._as_err('\'val_arr\' is empty!')
            ...
            for i, val in enumerate(val_arr):
                ...
                assert not isnan(gy_arr[i]), \
                    self._as_err('\'gy\' is nan (val: %0.2e)!' % val)
            ...
            assert val_arr.shape[0] > 1, self._as_err(
                ('\'val_arr\' has less than 2 elements! '
                 'min_prob: %f, max_prob: %f, val_arr: %s') % (
                         gy_arr.min(), gy_arr.max(), str(val_arr)))

            assert gy_arr.shape[0] > 0, \
                self._as_err('Increase discretization!')
            assert gy_arr.shape[0] == val_arr.shape[0], \
                self._as_err('Unequal shapes of probs and vals!')
            ...
            if len(gy_arr) == 1:
                ...
                for i, val_adj in enumerate(val_arr_adj):
                    ...
                    assert not isnan(gy_arr_adj[i]), \
                        self._as_err('\'gy\' is nan (val: %0.2e)!')
                    assert not isnan(pdf_arr_adj[i]), \
                        self._as_err('\'pdf\' is nan (val: %0.2e)!' % val_adj)
                ...
                assert pdf_arr_adj.shape[0] == val_arr_adj.shape[0], \
                    self._as_err('unequal shapes of densities and vals!')
        ...
        def _get_discharge_stuff():
            ...
            for i, prob in enumerate(probs_arr):
                ...
                gy_arr[i] = norm_cdf_py(_ / sig_sq_t**0.5)
                assert not isnan(gy_arr[i]), \
                    self._as_err('\'gy\' is nan (prob:%0.2e)!' % prob)
            ...
            _too_less = val_arr_adj.shape[0] > 1
            if not _too_less:
                self._as_err(
                        ('\'val_arr_adj\' has less than 2 elements! '
        ...
        out_add_info_df.loc[:, :] = [False, 0, 0, nan, nan]
        ...
        for curr_neb in self.curr_nrst_stns:
            ...
            comb_idxs_dict[curr_neb] = self.in_var_df.loc[:, _].dropna().index

        if self.use_best_stns_flag:
            avail_stns_per_step_dict = _get_unique_stns_seqs()
        ...
        for infill_date in infill_dates:
            ...
            if pre_avail_stns[1:] != avail_cols_raw:
                ...
                try:
                    ...
                    # TODO: find a better solution to print this
                ...
                assert curr_var_df.shape[1] > self.n_min_nebs, \
                    self._as_err(('\'curr_var_df\' has too few neighboring '
                                  'stations (%d) in it!' %
                                  curr_var_df.shape[1]))
                assert curr_var_df.shape[0] >= self.min_valid_vals, \
                    self._as_err('\'curr_var_df\' has too few records (%d)!' %
                                 curr_var_df.shape[0])
                ...
                if cov_vec.shape[0] == 0:
                    print '\n'
                    self._pretty_print([
                            'WARNING: \'cov_vec\' is empty on date:',
                            infill_date],
                            nbh=8,
                            nah=8)

                    self._pretty_print(['Stations in use are:'], nbh=12)
                    n_pas = len(pre_avail_stns)
                    for i_msg in range(0, n_pas, min(3, n_pas)):
                        self._pretty_print(self.curr_nrst_stns[i_msg:
                            (i_msg + 3)],
                            nbh=16)
                ...
                if sig_sq_t <= 0:
                    print '\n'
                    self._pretty_print(['WARNING:',
                                        (('Stn %s has an invalid conditional '
                                          'variance') % self.curr_infill_stn)],
                                       nbh=8,
                                       nah=8)

                    self._pretty_print([('\'sig_sq_t (%0.6f)\' is '
                                         'less than zero!') % sig_sq_t],
                                       nbh=12)

                    self._pretty_print(['\'infill_date\':', infill_date],
                                       nbh=12)

                    self._pretty_print(['\'best_stns\': \'covariance\''],
                                       nbh=12)
                    for bstn_cov in zip(best_stns.tolist(), cov_vec):
                        self._pretty_print(['%s: %0.7f' % (bstn_cov[0],
                                                           bstn_cov[1])],
                                           nbh=16)
                    self._pretty_print([''], nbh=24, nah=24)
                    print '\n'
            ...
            if self.infill_type == u'precipitation':
                assert not isnan(py_zero), self._as_err('\'py_zero\' is nan!')
            ...
            assert len(_) == len(avail_cols_fin) + 1, \
                self._as_err(('\'curr_val_cdf_ftns_dict\' has incorrect '
                              'number of keys!' +
                              str(curr_val_cdf_ftns_dict.keys()) +
            ...
            if np_any(descend_idxs):
                ...
                self._pretty_print([(('WARNING: '
                                   ...
                                   nbh=8,
                                   nah=8)
                self._pretty_print([self.conf_heads, list(conf_vals)], nbh=8)
                self._pretty_print(['\'gy\':\n', list(gy_arr_adj)], nbh=8)
                self._pretty_print(['\'theoretical_var_vals\':\n',
                                    list(val_arr_adj)],
                                   nbh=8)
                assert False, \
                    self._as_err(('Interpolated \'var_vals\' on %s not in '
                                  'ascending order!') % str(infill_date))
                print '\n'
    ...
    def __plot_ecops(self, infill_stns):
        ...
        for infill_stn in infill_stns:
            ...
            for other_stn in self.curr_nrst_stns:
                ...
                ranks_i = new_ser_i.rank(method=self._rank_method).values
                ranks_j = new_ser_j.rank(method=self._rank_method).values
                ...
                if prob_i.min() < 0 or prob_i.max() > 1:
                    assert False, \
                        self._as_err('\'prob_i\' values out of bounds!')
                if prob_j.min() < 0 or prob_j.max() > 1:
                    assert False, \
                        self._as_err('\'prob_j\' values out of bounds!')
                ...
                ecop_grid_ax.spines['left'].set_visible(False)
                ...
                gau_cop_ax.spines['left'].set_visible(False)
    ...
    def _plot_infill_ser(self, args):
        ...
        infill_ax.set_title(('Infilled values for station: %s' %
                             self.curr_infill_stn.encode('ascii', 'ignore').decode('ascii')))
        plt.grid()
        plt.legend(framealpha=0.5, loc=0)
