#import pyximport
#pyximport.install()
#import variograms_simple_kriging as vgsk
import numpy as np
...
#        yk_sh = self.yk.shape
#
#        assert len(xi_sh) == len(yi_sh) == len(zi_sh) == \
#               len(xk_sh) == len(yk_sh) == 1
#        assert xi_sh[0] == yi_sh[0] == zi_sh[0]
#        assert xk_sh[0] == yk_sh[0]
#        assert type(self.model) is str
#
#        self.in_count = xi_sh[0]
#        self.out_count = xk_sh[0]
#        self.zk = np.zeros(shape=(xk_sh[0]))
...
#            vg = submodel.split(' ')[1].split('(')[0].strip()
#            self.vgs.append(vg)
#            self.sills.append(float(submodel.split(' ')[0].strip()))
#            self.ranges.append(float(submodel.split('(')[1].split(')')[0]))
#            self.vg_ftns.append(self.all_vg_ftns[vg])
#
#        for i in xrange(self.in_count):
#            x1 = self.xi[i]
#            y1 = self.yi[i]
#            for j in xrange(self.in_count):
#                dist = vgsk.get_dist(x1, y1, self.xi[j], self.yi[j])
#                self.in_dists[i, j] = dist
#
#        self.in_vars = np.zeros(shape=(self.in_count, self.in_count))
#        for g in xrange(self.in_count):
#            for h in xrange(self.in_count):
#                for f in xrange(len(self.vg_ftns)):
#                    self.in_vars[g, h] += self.vg_ftns[f](self.in_dists[g, h], self.ranges[f], self.sills[f])
#
#        self.in_vars = np.concatenate((self.in_vars, [np.ones(self.in_count)]), axis=0)
#        self.in_vars = np.concatenate((self.in_vars, np.row_stack(np.ones(self.in_count+1))), axis=1)
#        self.in_vars[-1, -1] = 0
#        for k in xrange(self.out_count):
#            x_interp = self.xk[k]
#            y_interp = self.yk[k]
#            out_vars = np.zeros(shape=(self.in_count,))
#
#            for l in xrange(self.in_count):
#                out_dist = vgsk.get_dist(self.xi[l], self.yi[l], x_interp, y_interp)
#                for n in xrange(len(self.vg_ftns)):
#                    out_vars[l] += self.vg_ftns[n](out_dist, self.ranges[n], self.sills[n])
#
#            out_vars = np.concatenate((out_vars, [1]))
#            lambdas = np.linalg.solve(self.in_vars, out_vars)
#            self.zk[k] = np.sum((lambdas[:-1] * self.zi))
#
#        return None
...
class Variogram(object):
    ...
    def _vg_var_raw(self):
        ...
        for m in range(0, self._tot_pairs_a, self.nk):
            ...
            vg_h_list.append((n_mid, self._evg(n[:, 0], n[:, 1],
                              self.evg_name), n.shape[0], n[0, 2], n[-1, 2]))
    ...
    def sph_vg(self, h_arr, arg):
        ...
        b = h_arr**3 / (2 * arg[0]**3)
        sph_vg = (arg[1] * (a - b))
        sph_vg[np.where(h_arr > arg[0])] = arg[1]
        return sph_vg
    ...
    def _vg_get_vg(self):
        ...
        self.perm_r_list = np.unique(self.perm_r_list)
        ...
        for n in self.perm_r_list:
            perm = permutations(self.mix_vg_list, n)
